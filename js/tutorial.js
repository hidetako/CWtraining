...
export const TUTORIAL_STEPS = [
  {
    ...
    body: [
      'パドルは「短点側」と「長点側」の 2 枚のレバーでできています。どちらのマウスボタンに割り当てるかは利き手に合わせて変えられます。初期設定は右手用で、左クリックが短点です。',
      ...
      '画面右のパドル欄に、今どちらのボタンがどちらの要素かが表示されています。迷ったらそこを見てください。',
    ],
  ...
  },
  {
    ...
    title: '短点を連続で出す',
    body: [
      '短点側のレバー（画面右のパドル欄で確認できます）を押したままにしてください。押している間、キーヤーが正確な間隔で短点を打ち続けます。',
  ...
  },
  {
    ...
    title: '右手用と左手用',
    body: [
      'パドルは同じ向きのまま、右手で操作すると<strong>左のレバーに親指</strong>が、左手で操作すると<strong>右のレバーに親指</strong>が当たります。',
      'CW では親指を短点に割り当てるのが一般的です。そのため左手で打つ場合は、短点と長点の割り当てを右手用とは逆にする必要があります。',
      'ページ下部の設定欄にある「利き手」を切り替えると、この割り当てが入れ替わります。画面右のパドル欄の表示も一緒に変わるので、どちらのボタンが今どちらの要素かはそこで確認できます。',
      '親指を長点にしたい方は「親指側を長点にする」も使えます。利き手の設定と組み合わせて、4 通りの割り当てから選べます。',
      '一度決めたら変えないほうが上達します。指が順序を覚えてしまうためです。',
    ],
    goal: '「利き手」を自分に合う側にする（今のままでよければ、そのまま次へ）',
    check: null,
  },
  {
    id: 'word',
